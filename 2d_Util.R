...
cv_rkhs <- function(cv_folds=5, X_all, Xl_all, K, K_sqr, lambda_pen, stepL0, gamma=1.05, niter=1000, tol=10^(-6), A_true, A_var_est){
  ...
  for(cv_fold_index in 1:cv_folds){
    ...
    W_est <- opt_result[[which.min(fvalues)]]$par
...
fFPE.seq = fFPE(npca_component.max = npca_component.max,fpca.obj = fpca_max,totalT_train = totalT_train)   
    ...
    npca_component_selected <- which.min(fFPE.seq)
